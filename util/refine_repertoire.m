...
function refine_repertoire(handles,repertoireName) 
...
choice='Yes';
if exist(refined_repertoire_filename) || ~isempty(strfind(repertoire_filename,'+.mat'));
...
%                 bases_init(units)=bases_tmp(end-length(units)+1:end);                
                ...
                [bases, activations, err, NbChannels, NbPatternFrames, NbUnits, NbIter, ndx_V, datasetNameRefined] = repertoire_learning(handles, datasetName, NbUnits, NbChannels, NbPatternFrames, NbIterRefine, remove_ndx_refine, W_init);
                if isempty(bases)
                    ...
                    save(refined_repertoire_filename,'bases','activations','NbUnits','NbChannels','NbPatternFrames','NbIter','dataset_dir','remove_ndx_refine','ndx_V','datasetName');
