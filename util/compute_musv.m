...
for fnameID = 1:length(flist)
     ...
     if ~exist(gtfile,'file') || flag
         ...
         segment_samples=floor(info.SampleRate*40); % 10 seconds            
         ...
         try
             tmp=resample(segment(1:min(100,info.TotalSamples)),fs,info.SampleRate);
         catch        
         ...
         for segmentID = 1:nb_of_segments
             ...
             if exist('audioread')
                segment=audioread(audiofile,[audio_start audio_end]);
             else                 
                segment=wavread(audiofile,[audio_start audio_end]);
             end                    
             ...
             [syllable_data_segment,segment_start_rewind,~,nb_of_frames]=compute_musv_segment(handles,segment,info.SampleRate,GTB,frame_start,audiofile,resample_normal);
         ...
         [syllable_data, syllable_stats, filestats] = syllable_activity_file_stats(handles, audiofile, Nfft, syllable_data, TotNbFrames); 
         filestats.configpar=handles.config;
         
         save(gtfile,'syllable_data','syllable_stats','filestats');
