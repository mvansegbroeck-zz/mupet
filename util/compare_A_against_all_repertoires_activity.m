...
function compare_A_against_all_repertoires_activity(handles,repertoireNames,repertoireA) 
...
for barID=1:size(sim_scores_highact,1)
   bwqs = min(sim_scores_highact(barID, 4),sim_scores_highact(barID, 2));
   bwq = abs(sim_scores_highact(barID, 2) - sim_scores_highact(barID, 4));
