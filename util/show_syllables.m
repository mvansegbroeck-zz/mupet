...
if handles.syllable_stats{1,syllable_ndx} == -100
    inter_syllable_interval='_';
else
    inter_syllable_interval=sprintf('%.2f',handles.syllable_stats{1,syllable_ndx});
end
...
syllable_info_string3=sprintf('%s%s%s%s%s%s', ...
    ...
    sprintf('  inter-syllable interval: %s ms\n',inter_syllable_interval), ...
