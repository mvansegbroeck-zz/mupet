...
function [wts,cfreqs] = gammatone_matrix_linear(nfft, sr, nfilts, min_freq, max_freq, width, maxlen)

    if nargin < 2;    sr = 250000; end
    if nargin < 3;    nfilts = 64; end
    if nargin < 4;    min_freq = 35000; end
    if nargin < 5;    max_freq = 110000; end
    if nargin < 6;    width = 0.5; end
    if nargin < 7;    maxlen = nfft/2; end
    
    wts = zeros(nfilts, nfft);

    % ERB filters
    cfreqs = fix([min_freq-(max_freq-min_freq)/nfilts:(max_freq-min_freq)/nfilts:max_freq+(max_freq-min_freq)/nfilts]);
